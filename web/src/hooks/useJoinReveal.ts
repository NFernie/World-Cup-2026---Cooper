...
type UseJoinRevealOptions = {
  navigateOnComplete?: boolean
}

export function useJoinReveal(options?: UseJoinRevealOptions) {
  ...
  const startReveal = useCallback(
    async (poolId: string, assignedTeamId: string, speed: RevealSpeed = 'normal') => {
      const [
      ...
      if (cErr || aErr || mErr || tErr || !assigned) {
        if (navigateOnComplete) navigate(`/pools/${poolId}`)
        return
      }
      ...
      const otherAssignedIds = new Set(
        (members ?? [])
          ...
          .filter((id): id is string => Boolean(id) && id !== assignedTeamId),
      )
      ...
      setReveal({
        ...
        allTeams:
          carouselTeams.length > 0
            ? carouselTeams
            : [{ fifa_code: assigned.fifa_code, name: assigned.name }],
        poolId,
        spinTeamCount,
        speed,
      })
  ...
  const completeReveal = useCallback(
    (onDone?: () => void) => {
      ...
      if (navigateOnComplete && current) navigate(`/pools/${current.poolId}`)
      onDone?.()
      setReveal(null)
    },
    [navigate, navigateOnComplete],
